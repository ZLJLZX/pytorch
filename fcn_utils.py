...
def evaluate(model, data_loader, device, num_classes):
    ...
    with torch.no_grad():
        ...
        for image, target in data_loader:
            ...
            conf_mat.update(target.flatten(), predict.argmax(1).flatten())  #展平处理 取出概率最大的类别
...
def train_one_epoch(model, optimizer, data_loader, device, lr_scheduler, scaler=None):
    ...
    for image, target in data_loader:
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            predict = model(image)#含两部分  out/aux  [4,21,480,480]  batchsize classes height * width
...
class ConfusionMatrix(object):
    ...
    def update(self, target, predict):
        n = self.num_classes
        if self.mat is None:  #创建n*n的矩阵 21*21 全为0
            self.mat = torch.zeros((n, n), dtype=torch.int64, device=target.device)
        with torch.no_grad():
            k = (target >= 0) & (target < n)#产生一个mask  255的背景值为false
            inds = n * target[k].to(torch.int64) + predict[k]  #target k(不含255的矩阵)  n为类别     预测的类别数（predict）
